...
MEMORY_RECORDS = {
    ...
    "memory_md": "db memory",
    "soul_md": "db soul",
    # Legacy DB value should be ignored; AGENTS.md comes from the template.
    "agents_md": "db agents",
}
...
async def test_sync_memory_reads_all_files_concurrently_then_applies_sandbox_wins():
    ...
    memory_service.get_agents_template_content.return_value = "template agents"
    ...
    assert write_paths == ["/home/user/USER.md", "/home/user/MEMORY.md", "/home/user/AGENTS.md"]
    assert sandbox.files.write_file.await_args_list[-1].args[1] == "template agents"
...
async def test_sync_memory_writeback_failure_skips_that_file_and_continues():
    ...
    assert writes == {
        "/home/user/USER.md": "db user",
        "/home/user/AGENTS.md": "template agents",
    }
    memory_service.upsert_memory_file.assert_called_once_with("user-1", "soul_md", "sandbox soul")
    read_db.rollback.assert_called_once()
