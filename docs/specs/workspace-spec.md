...
- 工作区侧栏支持从本机拖入多个文件，复用菜单上传接口及同名冲突规则；拖到目录行上传至该目录，拖到文件行上传至其父目录，空白区域上传至根目录，搜索结果遵循相同落点规则。拖入时高亮目录并显示目标路径，上传期间显示文件名且不接受重复上传；成功条目即时刷新，失败显示错误。保留菜单选择文件入口，不递归上传本机文件夹，拖入文件夹时明确提示。外部文件上传不触发内部条目移动或会话附件上传。
...
- 工作区文件树左右由列表容器统一留白，行内不叠加外边距和左内边距；同层文件与目录共用 16px 展开槽，图标与复选框共用固定位置，子目录每级缩进 8px。右侧操作按钮固定 28px 宽且不收缩、不追加右外边距，将剩余宽度留给文件名；长名称省略并保留完整 title，悬停和多选切换时名称位置不跳动。
