...
- Composer 选择文件、粘贴文件、输入框及页面拖拽共用添加入口；对话图片在创建卡片、本地预览 URL 和上传入队前按当前模型的 `supports_image/max_images` 校验，数量包含已有的待上传附件。模型能力尚未加载时暂缓添加图片；不支持时提示“当前模型不支持图片输入，请切换至支持图片的模型后再上传。”，不上传、不生成附件卡片。超出图片数量上限时拒绝本批新增图片，同批非图片文件继续上传。
- 切换模型后从当前附件和模型能力即时派生兼容性，不改写上传状态或删除附件。不兼容图片卡片显示“不兼容”，附件区提示切换模型或移除图片；按钮及 Enter 均不可发送，图片重试也不可发起上传。每个队列请求真正启动前检查其所属草稿的最新模型；不兼容的等待图片留在队列，已发起的上传可正常完成，普通文件和其他草稿不受阻塞。切回兼容模型或移除冲突图片后恢复并自动续传等待项，已上传图片无需重传；移除附件或开始有效提交时清除旧的添加失败提示。发送构建内容块时仍复用能力校验，后端对实际图片输入保留权威校验。
- 上述限制只针对对话图片输入；独立 Workspace 上传以及作为 `source=workspace` 文件引用的图片继续作为工具文件处理，不占模型图片数量。
- Composer 文件通过添加校验后即建立稳定附件卡片和本地预览，最多三个上传并发；使用临时草稿上传接口，状态为等待、上传、保存、就绪、失败。Axios 字节进度达到 100% 只代表传输完成，后端保存成功才可发送；未知总量不显示伪百分比。失败只影响该文件，支持重试与移除，上传中可以继续输入或追加附件。
...
- 上传/图片兼容性、Skill/MCP 列表加载、消息发送及预览等错误提示使用统一 `FeedbackMessage`，显示 3 秒且支持手动关闭，遵循 frontend-spec §4.5.1 的暂停规则。再次触发相同错误时重新展示；同一提示不会因继续输入而延长。提示隐藏不删除草稿或附件、不修改上传/Run 状态，也不解除图片不兼容时的发送和重试限制；附件保留“不兼容”状态及可访问的原因说明。

| 错误 | 来源 | UI 表现 |
|---|---|---|
| `HttpError(4xx)` | fetch 拒绝 | definite rejection；显示错误 banner，**不自动重试** |
| `HttpError(5xx)` | fetch 拒绝 | definite rejection；显示错误 banner，允许用户显式重试，但不自动重发 |
| 受理前拒绝（含并发上限 `HTTP 429`） | HTTP / 无序 SSE 错误，尚无 durable Round | 一次性恢复正文、附件与本轮偏好，移除临时 Round；错误原因通过独立会话错误状态展示在输入框上方的 `role=alert`，不得依赖已删除的临时 Run 处理后续错误事件。429 明确显示当前并发上限与等待完成后重试的提示，不刷新页面、不自动重发、不创建失败 Round |
