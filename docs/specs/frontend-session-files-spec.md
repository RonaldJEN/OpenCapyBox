...
- 每个 session 在保持 `split` 时独立保留 splitter 比例；通过聊天顶栏临时收起再打开时恢复标签、路径和预览，但比例固定重置为聊天 45% / 文件 55%，不得继承上次的极窄比例。右上角 X 是“关闭所有文件”：同步抓取当前面板全部 dirty 内容交给既有 outbox 后，立即清空当前 owner 的标签与活动文件并关闭面板，不等待远端保存、不删除文件、不弹例行确认。下次展开 Session 文件区显示原目录列表，从文件卡再次进入时只打开本次目标，不恢复已关闭标签；其他 Session 的标签不受影响。工作区右上角 X 使用相同关闭全部语义。
...
- 文件区顶栏 X 的提示与无障碍名称为“关闭所有文件”，清空当前面板标签并关闭文件区；关闭后焦点返回“查看文件”。单个标签上的 X 仍只关闭该文件。

### 3.2 面板布局

- 聊天顶栏的 24×24 按钮在 `closed` 时显示“恢复分栏”，以聊天 45% / 文件 55% 的固定比例恢复右侧 `split`；`split` 时显示“只看聊天”，关闭右侧文件区并让聊天铺满。
...
- 文件区顶栏的对称按钮在 `split` 时显示“只看文件”，进入 `full`；在 `full` 时显示“恢复分栏”，返回 `split`，图标 14×14。
...
- `full`：文件工作台占满主内容区；聊天顶栏不渲染，消息、输入框与运行态仍保持挂载并隐藏。桌面在文件区最左边缘只保留一根共享 splitter。它覆盖导航/主内容的同一物理边界并按首次移动方向锁定：向左收起导航，向右立即恢复 `split`；同一手势即使反向移动也不得串到另一操作。
...
- 文件区打开、关闭、full、owner 切换与尺寸变化统一由聊天阅读位置管理器处理。阅读位置是 round_id + 正文块稳定标识 + 块内字符位置 + 该字符所在行的视口偏移，不得使用首开附件的 scrollTop、DOM 块序号或段落顶端旧像素偏移代替；Markdown 块标识由源节点起始位置产生，内联标签重建不改变字符定位。长段落缩短后仍须保留正在阅读的文字，嵌套列表定位实际可见文本所属块；非文本卡片/图片使用元素锚点。
- splitter 在手势开始且聊天仍可见时冻结阅读位置，每个 CSS 比例帧修改后恢复同一字符，手势结束才提交 React 比例；禁止拖至 0 后再从零宽几何捕获，禁止逐帧重新选择字符造成累计漂移。窗口尺寸、图片加载等异步重排由 ResizeObserver 通知同一个管理器恢复，不另设滚动写入者或延时补偿。原本跟随底部才保持底部。
- 进入 `full` 前保存该 Session 最后可见的文字位置，隐藏期间不读取几何；跨 Session 后回访仍为 full 时保留这一书签，恢复聊天时使用它。普通进入可见聊天仍遵循父级 spec 的定位最新消息语义。焦点恢复使用 `preventScroll`，不拥有滚动位置。
...
- 目录视图顶栏提供“下载当前文件夹（ZIP）”，文件夹行提供独立 ZIP 下载按钮，点击下载不进入目录。根目录下载为 `会话文件.zip`，子目录下载为 `目录名.zip`，包含完整目录内容而非搜索筛选结果。打包期间显示文件夹名称并禁用重复打包，失败保留错误及重试入口；状态按 Session 隔离。调用 `GET /api/sessions/{id}/files-archive?path=...`，沿用鉴权下载，普通文件下载不变。
- 同一路径最多一个标签。
...
- [ ] 聊天面板按钮在桌面 `closed/split` 始终存在：文件区关闭时“收起面板”缩小聊天区并打开右侧，文件区打开时“展开面板”关闭右侧并让聊天铺满，且不会改变全局 SessionList/历史侧栏
