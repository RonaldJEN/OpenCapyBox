...
- 写操作和连接测试的成功反馈使用统一、可关闭的短暂 toast：默认显示 4 秒；普通信息提示默认 5 秒。倒计时在鼠标悬停或键盘焦点进入提示时暂停，离开后继续。
- 警告和错误默认不自动消失，必须提供明确的“关闭提示”按钮；可重试的加载错误还应保留就近重试入口。错误使用 `role="alert"`，成功、信息和警告使用 `role="status"`。
- 业务状态（启用状态、工具数量、最近测试时间、降级状态、表单校验和凭证变化警告）与一次性操作反馈分离：前者随事实条件持续展示，后者不得占用页面常驻空间。
