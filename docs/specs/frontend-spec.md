...
- 登录入口拆分为 `/login`（用户工作台）和 `/admin/login`（管理后台），两者复用 `/auth/login`、同一账号体系和同一 Bearer Token；simple/LDAP 用户均可通过任一入口认证。
- `/login` 登录成功后一律进入用户工作台；管理员仍具备普通用户能力，可在用户工作台和管理后台之间切换且无需重新登录。
- `/admin/login` 必须明确标识“管理员登录”；仅在登录响应 `is_admin=true` 时保存新会话并进入 `/admin`。非管理员、禁用账号、错误凭证及 LDAP 内部错误在该页面统一显示“用户名或密码错误”，不得披露账号是否存在或是否具有管理员权限。
- 未登录或非管理员访问 `/admin` 时跳转 `/admin/login`；已登录管理员访问 `/admin/login` 时直接进入 `/admin`。
...
- 所有错误提示（包括上传、发送、加载、表单提交和弹窗内操作失败）与成功、普通信息反馈统一使用 `FeedbackMessage`：默认显示 3 秒，并提供明确的“关闭提示”按钮。倒计时在鼠标悬停或键盘焦点进入提示时暂停，离开后继续；不得为个别错误另设 5 秒或常驻展示。
- 警告保留手动关闭；错误使用 `role="alert"`，成功、信息和警告使用 `role="status"`。关闭或超时只隐藏反馈，不改变操作结果、失败状态或校验条件；可重试的加载错误在提示隐藏后仍保留就近重试入口。
- 业务状态（启用状态、工具数量、最近测试时间、降级、保存状态、附件不兼容、校验导致的按钮禁用和凭证变化警告）与一次性操作反馈分离：前者随事实条件持续展示，错误说明按上述提示生命周期隐藏，不能通过隐藏提示改变这些事实。
- 保活但通过 `hidden` 切换的设置分区在离开时必须清理该分区的一次性成功、警告和错误反馈，禁止用户返回分区时重新看到旧操作结果；编辑草稿等业务状态不受影响。
- 同一作用域的新操作开始时替换旧操作反馈。禁止同时使用 `window.alert` 和内联错误重复报告同一次失败。
- 同一错误再次触发时重新展示并从 3 秒开始计时；普通输入、进度刷新或 JSX 重渲染不延长当前提示。富文本提示以稳定消息身份管理计时与关闭，不能用每次渲染新建的元素对象作为同一提示的身份。
