...
通过 `manage_cron` 工具定义 Cron 任务，AI 助手自主定期执行：
...
- **个人 MCP**：用户在“数据连接”中自行维护，仅本人可见；默认只允许公网 HTTPS。管理员可配置统一的域名后缀/CIDR 白名单，为受信任的内部服务放开私网与 HTTP；localhost、云元数据及其他高风险地址始终禁止。
...
- MCP 工具默认采用 Deferred 暴露：初始请求不注入完整远端 schema，Agent 通过 `mcp_tool_search` 按需发现后才加载，隐藏工具和权限 `DENY` 工具不会出现在搜索结果中。
...
MCP_CONNECT_RETRY_ATTEMPTS=3 # 包含首次发送前连接尝试
