...
export function formatSendError(err: any): string {
  const message = extractErrorMessage(err);
  if (
    err?.status === 413
    || (err?.status === 422 && (message === '[object Object]' || message.includes('at most')))
  ) {
...
export function formatDownloadError(err: any): string {
  const message = extractErrorMessage(err);
